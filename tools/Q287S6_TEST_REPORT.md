...
**Date:** 2026-06-28
**Branch:** (fork branch q287s6-tools)
**Device under test:** real DEEBOT NEO 2.0 PLUS (`q287s6`), fw 10.0.6, real account
...
1. **Fan speed never surfaced.** The device's default `fanMode:"auto"` was not
   ...
   log unmapped values instead of dropping them. *(an earlier commit)*
2. **Water amount never surfaced.** Real wire value is `waterMode:"mid"`; the
   ...
   `WaterAmountEvent` was emitted. Fixed to `"mid"`. *(an earlier commit)*
3. **Resume and dock used the wrong surfaces.** Resume was inferred as
   ...
   {chargeSwitch:true}`. Both corrected and confirmed live. *(an earlier commit)*
